...
Use this checklist after maintainers create the intended first exact StateGate release and update the floating `v1` tag. These checks are intentionally consumer-facing and do not require rewriting historical `v1.0.0` provenance.

## Tag model

- `v1.0.0` is the immutable historical pre-StateGate release and must remain bound to source commit `b26c7c29b1f52ac78f6112f9b1a2f1180b00a600`.
- `v1.1.0` is the intended first exact StateGate release. Do not claim it exists until maintainers have actually created it.
- `v1` is the floating major tag. After the `v1.1.0` release is created, `v1` must resolve to the same release commit as `v1.1.0`.
...
- Inspect the floating, intended exact, and historical exact tags:
  ...
  ```bash
  git ls-remote --tags https://github.com/joselunasrt8-creator/stategate.git refs/tags/v1 refs/tags/v1.1.0 refs/tags/v1.0.0 'refs/tags/v1^{}' 'refs/tags/v1.1.0^{}' 'refs/tags/v1.0.0^{}'
  ```

- If `v1.1.0` has not been created yet, record that result as expected pre-release state and do not treat `v1` as verified for StateGate consumers.
- After `v1.1.0` exists, confirm `v1` and `v1.1.0` dereference to the same release commit.
- Confirm `v1.0.0` still dereferences to `b26c7c29b1f52ac78f6112f9b1a2f1180b00a600`.
...
- Verify `release/validator-metadata.json` for the published `v1.1.0` release uses `validator_version: "1.1.0"` and does not report `development`.
- Verify the `v1.1.0` release manifest source commit matches the exact `v1.1.0` tag target.
- Verify the historical `v1.0.0` manifest remains bound to source commit `b26c7c29b1f52ac78f6112f9b1a2f1180b00a600`.
...
2. Identify the intended `v1.1.0` target commit and verify its release manifest, validator metadata, changelog entry, and proof-contract tests.
3. Move only the floating `v1` tag back to the verified `v1.1.0` commit through the repository maintainer release process.
